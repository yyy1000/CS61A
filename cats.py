...
def accuracy(typed, reference):
    ...
    if typed_length == 0:
        if reference_length == 0:
            ...
            return 0.0
    min_length = min(typed_length,len(reference_words))
    count = 0
    for i in range(min_length):
        if typed_words[i] == reference_words[i]:
            count += 1
    return count / typed_length * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    words_length = typed_length / 5
    return words_length * 60 / elapsed
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        ...
        if first_meet and diff <= min_limit:
            min_limit = diff
            res_word = word
            first_meet = False
        elif not first_meet and diff < min_limit:
            min_limit = diff
            res_word = word
    return res_word
    # END PROBLEM 5
...
def pawssible_patches(start, goal, limit):
    ...
    start_len = len(start)
    goal_len = len(goal)
    if start_len==0 or goal_len==0:  # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        return max(start_len, goal_len)
        # END
...
def final_diff(start, goal, limit):
    ...
    '''
    pawssible_patches:
    Correction Speed: 130.85743799911157 wpm
    Correctly Corrected: 80 words
    Incorrectly Corrected: 15 words
    Uncorrected: 4 words

    sphinx_switches:
    Correction Speed: 34047.42421435828 wpm
    Correctly Corrected: 430 words
    Incorrectly Corrected: 398 words
...
def time_per_word(times_per_player, words):
    ...
    for j in range(player_num):
        now_times = list()
        times = times_per_player[j]
        time_len = len(times)
        for i in range(1, time_len):
            now_times += [times[i]-times[i-1]]
        times_per_player[j] = now_times
...
def fastest_words(game):
    ...
    for word_index in word_indices:
        ...
        res[min_player] += [word_at(game,word_index)]
    return res
    # END PROBLEM 10
...
def run_typing_test(topics):
    ...
    while True:
        ...
        elapsed = (datetime.now() - start).typed_seconds()
        print("Nice work!")
