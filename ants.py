...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        count = 0
        while not throw_place.is_hive():
            the_bee = bee_selector(throw_place.bees)
            if the_bee and self.min_range <= count <= self.max_range:
                return the_bee
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        ...
        self.chewing = 0
    def action(self, gamestate):
        find_maybee = bee_selector(self.place.bees)
        if find_maybee:
            if self.chewing == 0:
                self.chewing = self.chew_duration
                find_maybee.reduce_health(find_maybee.health)
            else:
                self.chewing -= 1
# END Problem 7
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(health)
        QueenAnt.queen_id = QueenAnt.queen_id + 1
        self.queen_id = QueenAnt.queen_id
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.queen_id > 1:
            ...
            while the_place:
                if the_place.ant:
                    the_place.ant.buff()
                    if the_place.ant.is_container() and the_place.ant.contained_ant:
                        the_place.ant.contained_ant.buff()
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.queen_id==1 and self.health <= amount:
            bees_win()
        else :
            super().reduce_health(amount)
        # END Problem EC
